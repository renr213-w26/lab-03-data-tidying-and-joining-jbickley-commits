...
The country_inflation table has 44 rows, and 30 columns. The columns represent years and the rows represent countries.

b.  Display a list of the countries included in the dataset.

```{r}
country_inflation |> pull(country) |> unique()
```
...
```{r}
top_inflation_2023 <- country_inflation |> select(country,`2021`) |> 
  arrange(desc(`2021`)) |> head(n= 3)
```

Argentina, Turkey, and Brazil had high inflation rates of 48%, 20%, 8% all far above the US's rate of 4.9% with them being over 9X, 4X, and 1.5X their inflation.
...
```{r}
inf_ratio <- country_inflation |> select(country,`1993`,`2021`) |> 
  mutate(inf_ratio2 = `2021`/`1993`) |> arrange(desc(inf_ratio2)) |> 
  select(country,`1993`,`2021`)
inf_ratio 
```

The country with the largest relative increase in inflation if New Zealand. The country with the larges absolute change in inflation is Brazil, where inflation fell for 1927.38% to 8.3%
...
```{r}
pivoted_inflation <- country_inflation |> 
  pivot_longer(-country,names_to = "year", names_transform = as.numeric, values_to= "inflation")
pivoted_inflation

```

There are `r nrow(pivoted_inflation)` rows and `r ncol(pivoted_inflation)` columns. ::: callout-important The remaining questions in Part 1 require the use of the pivoted data frame from Question 4. :::
...
```{r}
pivoted_inflation |> filter(inflation == max(inflation, na.rm = TRUE))
```

The country with the highest inflation between 1993 and 2021 is Brazil in 1994 at 2075.9%
...
```{r}
pivoted_inflation |> filter(inflation == min(inflation, na.rm = TRUE))

```

The lowest reported infaltion rate between 1993 and 2021 is Ireland in 2009 with a re of -4.48%
...
```{r}
pivoted_inflation |>
  filter(inflation == min(inflation, na.rm = TRUE) | inflation == max(inflation, na.rm = TRUE))
...
countries_of_interest <- c(
  "Canada", "Brazil", "Ireland", "China (People's Republic of)", "United States"
)
```

So on and so forth... Then, in 1-2 sentences, state why you chose these countries.

These countries were chosen firstly with countries with standout rates, like Brazil and Ireland who had the highest and lowest inflation rates at some point; Canada and US as reference as I am most familiar with these countries inflation situations and China due to large economic power.
...
```{r}
of_interest <- pivoted_inflation |> filter(country %in% countries_of_interest)
of_interest |> distinct(country)
```
...
```{r}
of_interest |> ggplot(aes(year, inflation, color = country)) + 
  geom_line()+
  geom_point(aes(shape = country)) + 
  scale_color_viridis_d() +
  theme_gray()+
  labs(title = "Inflation from 1993 to 2021 in Selected Countries", y = "Inflation %")

of_interest |> filter(year > 1995) |> ggplot(aes(year, inflation, color = country)) + 
  geom_line()+
  geom_point(aes(shape = country)) + 
    scale_color_viridis_d() +
  theme_gray()+
  labs(title = "Inflation from 1995 to 2021 in Selected Countries", y = "Inflation %")
  
```

I produced 2 graphs because Brazil's inflation in 1994 is so extreme the graph becomes hard to read so one is all years and is starting in 1995. Some interesting thigs that we can see in the post 1995 one is all the countries, though very different tend to trend in the same direction the same time, lowest points are all very similar in times across all 5 countries.
...
The US inflation data set has `r nrow(us_inflation)` rows which represent division numbers for each year and `r ncol(us_inflation)` columns contain the data.
...
The US inflation data set has `r nrow(cpi_divisions)` rows which represent each division and `r ncol(cpi_divisions)` columns contain the information about each one.
...
-   Use a short but informative name for the joined dataset, and do not overwrite either of the datasets that go into creating it.
    ...
    ```{r}
    us_inflation_w_divisions <- us_inflation |>
...
This data set has `r nrow(us_inflation_w_divisions)` rows which represent division numbers for each year and `r ncol(us_inflation_w_divisions)` columns contain the data and now has a description of the division that entry is for.
...
These divisions (Food, Clothing, Housing, Transportation, and Education) were chosen because they make up of household spending and are most relevant to visualize.
...
```{r}
div_of_interest <- us_inflation_w_divisions |> 
  filter(cpi_division_id %in% divisions_of_interest)
div_of_interest |> distinct(cpi_division_id)
...
```{r}
div_of_interest |> ggplot(aes(year, annual_inflation, color = description))+
  geom_line()+  
  geom_point(aes(shape = description))+
  labs(title = "Inflation In the US by Sector", y = "Inflation %", color = "Description", 
       shape = "Description")+
  theme_dark()+
...
legend.key = element_rect(fill = "grey", colour = NA)
  )
```

For the most part the different categories stay fairly close together, with a notable exception being transportation which is more volatile. This makes sense as its very exposed to commodity prices. The inflation is also relatively flat across all groups over time.
