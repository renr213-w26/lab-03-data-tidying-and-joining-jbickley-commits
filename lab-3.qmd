...
```{r}
top_inflation_2023 <- country_inflation |> select(country,`2021`) |> arrange(desc(`2021`)) |> head(n= 3)
```
...
```{r}
inf_ratio <- country_inflation |> select(country,`1993`,`2021`) |> mutate(inf_ratio2 = `2021`/`1993`) |> arrange(desc(inf_ratio2)) |> select(country,`1993`,`2021`)
inf_ratio 
```
...
```{r}
pivoted_inflation <- country_inflation |> pivot_longer(-country,names_to = "year", names_transform = as.numeric, values_to= "inflation")
pivoted_inflation

```
There are `nrow(pivoted_inflation)` rows and `ncol(pivoted_inflation)` columns.
::: callout-important
...
```{r}
pivoted_inflation |> filter(inflation == min(inflation, na.rm = TRUE) | inflation == max(inflation, na.rm = TRUE))
